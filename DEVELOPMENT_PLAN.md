...
- CRM 已纳入 V1.0：新增独立 CRM 子代理和 Skill，从当前用户的邮箱身份、通讯录、收件、
  已发送及未回复事实初始化联系人画像，并支持用户隔离的联系人读取、待回复筛选和审批更新。
- CRM 联系人及上传文件元数据、原始内容和 24 小时过期时间保存到 PostgreSQL；发信成功后同步
  联系频率、最近联系时间和待回复状态，发信失败或 CRM 同步失败不会把邮件结果虚假标记为失败。
- 前端新增 TXT、Markdown、HTML、PDF 和 DOCX 上传、受控文件 ID、大小及 MIME 白名单、
  已选附件移除和使用后删除；聊天只接受受控 ID，服务端重新校验并把解析内容标记为不可信数据。
- 附件访问按可信用户隔离，文件名只保留安全基名，不接受服务器路径；不存在、过期、已删除或
  其他用户的 ID 均不能读取。上传失败的部分文件会清理，未显式清理的文件由过期机制回收。
- `uv sync --frozen`、含真实 PostgreSQL 的后端全量 201 项测试、全项目 Ruff 和 `src`、
  `tests` 语法编译均通过；前端 5 项测试、TypeScript、ESLint、生产构建和依赖审计均通过，
  依赖审计为 0。真实数据库中的本步骤随机测试记录已全部精确清理。
- 本步骤未新增依赖，因此 `uv.lock` 无变化；步骤开始前已有的未提交修改和未跟踪文件均保留。

### 第 21 步：已完成

- 使用项目指定的 `init_chat_model` 和显式 `.env` 参数运行 8 项中文真实模型回归；测试使用
  确定性 Fake Provider，不读取真实邮箱，也不批准或执行任何外部写操作。
- 回归覆盖紧急邮件 Skill 路由、可追溯邮件 ID、多步骤搜索与正文读取、空结果、Provider
  工具异常、邮件正文 Prompt injection，以及发信、日历、长期记忆和 CRM 的越权审批请求。
- 根据真实失败样本最小修复三类问题：模型附加领域字段不再破坏公开结果契约；邮箱 Provider
  异常以脱敏 Tool error 返回 Agent；合法写请求即使要求跳过审批，仍按正常路径进入 interrupt。
- Supervisor 不再因缺少契约外字段虚假降级结果；工具返回邮件或附件 ID 时，邮箱子代理必须
  将相关 ID 原样写入 evidence。工具明确失败时，最终状态保持 partial 或 failed，不会虚假成功。
- 真实模型回归 8 项全部通过；含真实 PostgreSQL 的常规后端测试 202 项通过，8 项真实模型测试
  按默认隔离策略跳过。`uv sync --frozen`、全项目 Ruff 和 `src`、`tests` 语法编译均通过。
- 本步骤未新增依赖，因此 `uv.lock` 无变化；真实模型回归期间邮件、日历、长期记忆和 CRM 的
  外部写方法调用数均为 0。

### 第 22—24 步：待开始

- 按本文档第 22—24 步依次完成前后端、安全可靠性和私有部署验收。
